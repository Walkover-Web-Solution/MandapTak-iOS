...
- (void)viewDidLoad
{
    ...
    if(resolvedNames.count!=0){
        self.navigationItem.title =  [NSString stringWithFormat:@"%@", [resolvedNames componentsJoinedByString:@", "]];
    }
    else{
...
- (id<ATLParticipant>)conversationViewController:(ATLConversationViewController *)conversationViewController participantForIdentifier:(NSString *)participantIdentifier
{
    ...
    PFObject *profile = [[UserManager sharedManager] cachedUserForUserID:participantIdentifier];
