...
@property (nonatomic) NSCache *userCacheProfile;
...
- (void)queryAndCacheUsersWithIDs:(NSArray *)userIDs completion:(void (^)(NSArray *, NSError *))completion
{
    ...
    [query1 whereKey:@"objectId" containedIn:userIDs];
    
    [query1 findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error) {
            PFQuery *query = [PFQuery queryWithClassName:@"Profile"];
            [query whereKey:@"userId" containedIn:objects];
...
- (void)cacheUserIfNeeded:(PFObject *)user
{        PFUser *userObj  =[user valueForKey:@"userId"];
...
- (NSArray *)resolvedNamesFromParticipants:(NSArray *)participants
{
    ...
    for (NSString *userID in participants) {
        if ([userID isEqualToString:[PFUser currentUser].objectId]) continue;
        if ([self.userCache objectForKey:userID]) {
            PFObject *user = [self.userCache objectForKey:userID];
            [array addObject:[user valueForKey:@"name"]];
        }
    }
    return [NSArray arrayWithArray:array];
}
